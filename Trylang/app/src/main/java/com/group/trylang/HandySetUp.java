...
public class HandySetUp extends AppCompatActivity {
    ...
    private Toolbar handy_toolbar;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_handy_set_up);
        overridePendingTransition(android.R.anim.fade_in, android.R.anim.fade_out);

        circleImageView = findViewById(R.id.profile_pic);
        name = findViewById(R.id.handy_name);
        age = findViewById(R.id.handy_age);
        gender = findViewById(R.id.handy_gender);
        address = findViewById(R.id.handy_address);
        contact = findViewById(R.id.handy_contact);

        nexted =  findViewById(R.id.next);

        handy_toolbar = findViewById(R.id.handy_setup);
        setSupportActionBar(handy_toolbar);
        getSupportActionBar().setTitle("Profile Set Up");
        ...
        firestore.collection("Handyman").document(Uid).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    if (task.getResult().exists()) {
                        ...
                        Glide.with(HandySetUp.this).load(imageUri).into(circleImageView);
                    }
                }
            }
        });
        ...
        save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(!client_name.isEmpty() && !client_address.isEmpty() && !client_contact.isEmpty() && !client_age.isEmpty() && !client_gender.isEmpty() &&  imgUrl!= null){
                    showPD();
                    StorageReference postRef = storageReference.child("Handyman Profiles").child(FieldValue.serverTimestamp().toString() + ".jpg");
                            ...
                            if(task.isSuccessful()){
                                postRef.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                                    ...
                                    public void onSuccess(Uri uri) {
                                        ...
                                        firestore.collection("Handyman").document(Uid).set(postMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                            @Override
                                            public void onComplete(@NonNull Task<Void> task) {
                                                if(task.isSuccessful()){
                                                    ...
                                                    toast.show();
                                                    startActivity(new Intent(HandySetUp.this, HandyDashboard.class));
                                                    finish();
                                                }else{
                                                    progressDialog.dismiss();
                                ...
                                progressDialog.dismiss();
                                Toast.makeText(HandySetUp.this, task.getException().toString(), Toast.LENGTH_SHORT).show();
                            }

                        }
                    });
                }else{
                    Toast.makeText(HandySetUp.this, "Please check the fields required!", Toast.LENGTH_SHORT).show();
                }
